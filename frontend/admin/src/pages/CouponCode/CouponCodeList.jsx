...
function CouponCodeList() {
    ...
    async (page, limit, search = "") => {
      try {
        ...
        const response = await getData(
          `${ApiEndPoint.getCouponCodeList}?${queryParams.toString()}`
        );

        if (response.body) {
          setCouponList(response.body.rows || []);
  ...
  useEffect(() => {
    fetchCoupons(currentPage, entriesPerPage, debouncedSearchQuery);
  ...
  const handleDeleteCoupon = async (id) => {
    Swal.fire({
      ...
      if (r.isConfirmed) {
        try {
          await deleteData(`${ApiEndPoint.deleteCouponCode}/${id}`);
          Swal.fire({
            icon: "success",
            title: "Deleted!",
            timer: 1500,
  ...
  const handleAddCoupon = () => navigate("/couponCode_add");
                ...
                {loading ? (
                  ...
                  couponList.map((item, index) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900">
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 tracking-widest uppercase">
                          {item.code || "N/A"}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-900">
                      ...
                      <td className="px-6 py-4 text-sm font-medium space-x-3">
                        <button
                          onClick={() => navigate(`/couponCode_edit/${item.id}`)}
