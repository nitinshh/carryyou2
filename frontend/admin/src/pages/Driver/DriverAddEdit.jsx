...
function DriverAddEdit() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      dataToSend.append("role", 2); // ✅ role 2 for driver
      ...
      if (response?.code === 200) {
        ...
        navigate("/driver_listing");
      } else {
        toast.error(`Failed to ${isEditMode ? "update" : "create"} driver`);
      }
    } catch (err) {
      console.error("Save error:", err);
      toast.error(`Error ${isEditMode ? "updating" : "creating"} driver`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm max-w-2xxl mx-auto mt-3" style={{ marginLeft: "12px" }}>
      ...
      <div className="flex flex-col items-center mb-6">
        <label htmlFor="driverImageUpload" className="cursor-pointer group relative">
          <img
